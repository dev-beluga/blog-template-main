---
title: 'LLM Zoomcamp Module 1: Building My First RAG System'
...
# LLM Zoomcamp Module 1: Building My First RAG System
...
### 3. Generation (OpenAI API)

Finally, we send both the user's question and the retrieved context to an LLM (OpenAI's GPT models) to generate a comprehensive answer.
...
- **Context length limits**: Sometimes the retrieved documents were too long for the LLM's context window
- **Relevance tuning**: Getting the right balance between precision and recall in search results
- **Cost management**: API calls can add up quickly during development
